...
object roque {
	...
	method guardarAlimento(comida){
		 ...
		 */
	    if(comida != null && comidaDeAves == null){ 
	    // si hay comida alli && comidaDeAves no tiene nada
			game.removeVisual(comida) 
			// quito visual
		}else if(comidaDeAves != null && comida != null){
		// si tenemos una comida guardada & hay comida alli
			game.removeVisual(comida)
			// quito la visual de la comida
			game.addVisualIn(comidaDeAves, posicion.right(1)) 
			// lo ponemos a la derecha para evitar el loop de cuando queda en el mismo lugar
			//-------------------------------------------------
	...
	method darDeComerA(ave){
		if(comidaDeAves != null){ 
		// si tenemos comida
			ave.come(comidaDeAves) 
			// alimentamos
			game.addVisualIn(comidaDeAves, random)
			// hacemos que la imagen de dicha comida re aparezca en un lugar aleatorio
			comidaDeAves = null 
			// vaciamos nuestro storage de comida
		}
	}
}